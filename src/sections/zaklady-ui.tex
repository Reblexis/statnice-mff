...
\end{tema}
\begin{vezkratce}
\zk{Definice}{Prohledávání řeší cestu ve stavovém prostoru pomocí fronty kandidátů a testu cíle.}
\zk{Spouštěč}{stav, akce, BFS, DFS, UCS, A*, heuristika, přípustná, konzistentní}
\zk{Šablona}{Formuluj problém - vyber frontier pravidlo - řekni úplnost a optimalitu.}
\zk{Past}{A$^*$ v grafu potřebuje konzistentní heuristiku, jinak může uzavřít špatnou cenu.}
\end{vezkratce}
...
\end{tema}
\begin{vezkratce}
\zk{Definice}{CSP hledá hodnoty proměnných z domén tak, aby splnily všechny relační podmínky.}
\zk{Spouštěč}{proměnná, doména, omezení, hranová konzistence, AC-3, backtracking, MAC}
\zk{Šablona}{Zapiš proměnné a domény - proškrtni podporu AC-3 - dohledej MACem.}
\zk{Past}{Hranová konzistence může zachovat nesplnitelný problém, není to důkaz řešení.}
\end{vezkratce}
...
\end{tema}
\begin{vezkratce}
\zk{Definice}{Logické uvažování odvozuje důsledky z formulí pomocí modelů nebo pravidel.}
\zk{Spouštěč}{výrok, predikát, KB, entailment, SAT, CNF, rezoluce, unifikace}
\zk{Šablona}{Formalizuj znalosti - převeď tvar - použij splnitelnost, rezoluci nebo model.}
\zk{Past}{Důsledek $KB\models\alpha$ není totéž jako jen nalezení jednoho modelu s $\alpha$.}
\end{vezkratce}
...
\end{tema}
\begin{vezkratce}
\zk{Definice}{Pravděpodobnostní uvažování počítá nejisté závěry přes rozdělení a podmíněnosti.}
\zk{Spouštěč}{Bayes, nezávislost, síť, inference, skrytá proměnná, Markov, HMM, filtr}
\zk{Šablona}{Urči proměnné - rozlož společné rozdělení - marginalizuj a normalizuj.}
\zk{Past}{Podmíněná nezávislost závisí na daných proměnných, neplatí automaticky globálně.}
\end{vezkratce}
...
\end{tema}
\begin{vezkratce}
\zk{Definice}{Reprezentace znalostí zapisuje svět tak, aby šlo odvozovat změny, stav a nejistotu.}
\zk{Spouštěč}{situační kalkulus, rámec, Markov, filtrace, predikce, vyhlazování, HMM, DBN}
\zk{Šablona}{Zvol reprezentaci - zapiš stav a přechod - proveď odvození v čase.}
\zk{Past}{Markovský předpoklad omezuje závislost na aktuální stav, ne na celou historii.}
\end{vezkratce}
...
\end{tema}
\begin{vezkratce}
\zk{Definice}{Plánování hledá posloupnost akcí, která z počátečního faktického stavu splní cíl.}
\zk{Spouštěč}{akce, operátor, precondition, effect, dopředné, zpětné, plán}
\zk{Šablona}{Popiš stavy a akce - ověř předpoklady - skládej plán a kontroluj cíle.}
\zk{Past}{Akci lze použít až po splnění předpodmínek, efekty samy nestačí.}
\end{vezkratce}
...
\end{tema}
\begin{vezkratce}
\zk{Definice}{MDP modeluje rozhodování se stochastickými přechody, odměnami a optimalizací hodnoty.}
\zk{Spouštěč}{stav, akce, odměna, politika, Bellman, hodnota, value iteration, policy iteration}
\zk{Šablona}{Zapiš $S,A,T,R,\gamma$ - napiš Bellmana - iteruj hodnoty nebo politiku.}
\zk{Past}{Optimální akce maximalizuje očekávaný návrat, ne okamžitou odměnu.}
\end{vezkratce}
...
\end{tema}
\begin{vezkratce}
\zk{Definice}{Teorie her řeší interakci více agentů s užitky, strategiemi a rovnováhami.}
\zk{Spouštěč}{hra, strategie, minimax, alfa-beta, Nash, užitek, nula-součtová, aukce}
\zk{Šablona}{Urči hráče a užitky - vyber model hry - spočti strategii nebo rovnováhu.}
\zk{Past}{Minimax platí pro nula-součtové střídavé hry, ne pro každou strategickou hru.}
\end{vezkratce}
...
\end{tema}
\begin{vezkratce}
\zk{Definice}{Strojové učení odhaduje model z dat a hodnotí generalizaci na neviděných příkladech.}
\zk{Spouštěč}{supervised, unsupervised, reinforcement, strom, regrese, SVM, Bayes, EM, Q-učení}
\zk{Šablona}{Urči typ úlohy - řekni, co se učí - popiš data, trénink a evaluaci.}
\zk{Past}{Vysoká přesnost na tréninku neznamená dobrý model, může jít o přeučení.}
\end{vezkratce}
