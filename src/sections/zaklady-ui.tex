...
\begin{itemize}[leftmargin=1.2em,itemsep=1pt,topsep=1pt]
  ...
  \item \textbf{Uniform-cost search}: prioritní fronta podle dosavadní ceny $g(n)$. Je Dijkstra nad stavovým prostorem, optimální pro nezáporné ceny.
...
\noindent\bod{1}\ \textbf{Minimum:} definuj stav, akce, počáteční stav, test cíle a cenu. Vysvětli rozdíl tree search vs. graph search a pojmenuj DFS, BFS, UCS, A$^*$.\par
\noindent\bod{2}\ \textbf{+ k tomu:} u každého algoritmu řekni datovou strukturu frontier a optimálnost: BFS pro jednotkové ceny, UCS pro nezáporné ceny, A$^*$ pro přípustnou heuristiku ve stromu a konzistentní heuristiku v grafu.\par
...
\begin{tikzpicture}[>=Stealth, node distance=16mm,
  ...
  \node[dom,anchor=west] at (3.3,-1.25) {po hraně $A<B$:\\$D_A=\{3\}$\\$D_B=\{4\}$};
\end{tikzpicture}
\end{center}
...
\begin{odpoved}
\setlength{\parskip}{2pt}
\noindent\bod{1}\ \textbf{Minimum:} CSP = proměnné, domény, podmínky, úplné konzistentní přiřazení. AC-3 škrtá hodnoty bez podpory u sousedních proměnných.\par
\noindent\bod{2}\ \textbf{+ k tomu:} popiš frontu orientovaných hran v AC-3 a MAC jako backtracking plus opakované spuštění AC po každém přiřazení. Přidej mini příklad $A<B$.\par
...
\begin{odpoved}
\setlength{\parskip}{2pt}
\noindent\bod{1}\ \textbf{Minimum:} řekni úplná sdružená distribuce, nezávislost, Bayesovo pravidlo, vysčítání a normalizace. Bayesovská síť = DAG plus podmíněné tabulky.\par
\noindent\bod{2}\ \textbf{+ k tomu:} napiš faktorizaci sítě a ukaž ji na malé síti. Popiš enumeraci, eliminaci proměnných a rozdíl mezi zamítáním a vážením věrohodností.\par
...
\begin{odpoved}
\setlength{\parskip}{2pt}
\noindent\bod{1}\ \textbf{Minimum:} situační kalkulus = logika akcí a situací, problém rámce = jak nevyjmenovat všechny nezměněné fakty. HMM = skrytý stav, pozorování, přechodový a senzorový model.\par
\noindent\bod{2}\ \textbf{+ k tomu:} rozliš filtraci, predikci, vyhlazování a nejpravděpodobnější průchod. Nakresli řetěz $X_1\to X_2\to X_3$ s pozorováními $E_t$.\par
...
\begin{tikzpicture}[>=Stealth, node distance=22mm,
  ...
  \node[draw=none,align=left,anchor=west,font=\scriptsize] at (-0.7,-1.4)
    {$U(S_0)\leftarrow \max_a \sum_{s'}P(s'\mid S_0,a)[R+\gamma U(s')]$};
\end{tikzpicture}
\end{center}
...
\begin{odpoved}
\setlength{\parskip}{2pt}
\noindent\bod{1}\ \textbf{Minimum:} MDP = stavy, akce, přechody, odměny, strategie. Cíl je maximalizovat očekávaný diskontovaný užitek.\par
\noindent\bod{2}\ \textbf{+ k tomu:} napiš Bellmanovu rovnici a popiš iteraci hodnot i iteraci strategií. Uveď malý přechod se dvěma možnými následníky.\par
...
\begin{odpoved}
\setlength{\parskip}{2pt}
\noindent\bod{1}\ \textbf{Minimum:} rozliš učení s učitelem, bez učitele a zpětnovazební. Definuj rozhodovací strom, regresi, SVM a RL slovníkem vstup, výstup, cíl.\par
