...
\begin{itemize}[leftmargin=1em,itemsep=1.5pt,topsep=2pt]
  \item \textbf{Prohledávání:} stavový prostor; BFS/DFS/uniform-cost; A* je optimální s přípustnou heuristikou ve stromu a konzistentní v grafu.
...
\begin{itemize}[leftmargin=1em,itemsep=1.5pt,topsep=2pt]
  ...
  \item \textbf{Lineární regrese:} $y=w^\top x+b$, minimalizuj MSE; normální rovnice $w=(X^\top X)^{-1}X^\top y$ při regulární $X^\top X$.
  ...
  \item \textbf{SVM:} nadrovina s největším marginem; podpůrné vektory; měkký margin pro chyby, jádro pro nelineární hranice.
