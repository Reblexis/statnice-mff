...
je jejich součet). Pythagorova věta a Cauchy-Schwarzova nerovnost $|\langle u|v\rangle|\le\|u\|\|v\|$.\par
...
antiřetězec (žádné dva); \emph{Dilworthova věta} dává vztah šířky a pokrytí řetězci.
...
$\iff$ pro každou $X\subseteq A$ platí $|N(X)|\ge|X|$ (Hallova podmínka); ekvivalentně
\emph{Kőnig-Egerváryho věta}: max.\ párování = min.\ vrcholové pokrytí. Aplikace inkluze-exkluze:
počet surjekcí, Eulerova funkce, problém šatnářky. Počet prostých funkcí $N\to M$ je
$m^{\underline n}$ a bijekcí na $n$ prvcích je $n!$. ČUM má minimální a maximální prvky,
výška krát šířka je aspoň $|X|$.
\end{odpoved}
\begin{recall}
\begin{itemize}[leftmargin=1.2em,itemsep=0pt,topsep=1pt]
...
cesta mezi každými dvěma vrcholy v obou směrech, slabě souvislý = souvislý podkladový graf.
Eulerovský graf je souvislý a všechny stupně jsou sudé; v orientovaném grafu potřebuje
vyvážené vstupní a výstupní stupně. Kuratowski: nerovinný graf obsahuje dělení $K_5$ nebo
...
logika je rozhodnutelná (SAT), predikátová obecně ne; \emph{kompletní} teorie (rozhoduje o každé
formuli) jsou rozhodnutelné, ale např.\ Peanova aritmetika kompletní není.
