...
tečny). \emph{Integrál} = plocha pod grafem. Derivace a integrál jsou navzájem inverzní
...
Derivace $f'(a)=\lim_{h\to0}\frac{f(a+h)-f(a)}{h}$. Riemannův integrál = plocha pod grafem.\par
...
$\sum\frac1n$ diverguje. \emph{l'Hospital}: $\frac00,\frac\infty\infty \Rightarrow$ podíl derivací.
Průběh: $f'>0$ roste, $f''>0$ konvexní, extrém kde $f'=0$. \emph{Newton-Leibniz}:
...
\emph{Pozitivně definitní} ($x^\top Ax>0$) $\iff$ kladná vl.\ čísla $\iff$ existuje Choleského
rozklad $A=LL^\top$.
\end{odpoved}
\begin{recall}
\begin{itemize}[leftmargin=1.2em,itemsep=0pt,topsep=1pt]
...
plyne max.\ $3v-6$ hran. \emph{Barevnost} $\ge$ klikovost. \emph{Tok}: respektuje kapacity a
zachování; \emph{Ford-Fulkerson} = opakuj zlepšující cesty v reziduální síti; max tok = min řez.\par
\noindent\bod{3}\ \textbf{Bonus:} \emph{Mengerova věta}: počet hranově (vrcholově) disjunktních
cest = velikost minimálního hranového (vrcholového) řezu. Orientovaný graf: silně souvislý =
...
\end{itemize}
\end{recall}
\past{Eulerova formule platí pro souvislý rovinný graf. Bipartitní $\iff$ bez liché kružnice.}
...
$\mathrm{var}(X)=E[X^2]-E[X]^2$; pro nezávislé $\mathrm{var}(X+Y)=\mathrm{var}X+\mathrm{var}Y$.
\emph{Markovova nerovnost} $P(X\ge a)\le E[X]/a$.\par
\noindent\bod{3}\ \textbf{Bonus:} \emph{Zákon velkých čísel}: průměr $\to E[X]$. \emph{CLV}: průměr
...
hledání modelu. \emph{Věta o úplnosti}: dokazatelné $=$ platné.\par
