...
\noindent\bod{1}\ \textbf{Minimum:} \emph{Klasifikace} učí funkci $h:X\to Y$ s diskrétními třídami, \emph{regrese} učí $h:X\to\mathbb R^k$.
\emph{MLE} volí hypotézu nebo parametry $\hat h=\arg\max_h P(D\mid h)$.
Accuracy $=\frac{TP+TN}{TP+TN+FP+FN}$, precision $=\frac{TP}{TP+FP}$, recall $=\frac{TP}{TP+FN}$, $F1=\frac{2PR}{P+R}$.\par
...
\noindent\bod{3}\ \textbf{Bonus:} Regrese se hodnotí například $MSE=\frac1n\sum_i(y_i-\hat y_i)^2$ a $RMSE=\sqrt{MSE}$.
Minimalizace záporné log-věrohodnosti je často přesně ta loss funkce.
...
$\frac1n\sum_i (y_i - w^\top x_i-b)^2$, ekvivalentně L2 chybu $\|Xw-y\|_2^2$ (metoda nejmenších čtverců).\par
...
Funguje přímo jen při regulární $X^\top X$; při singularitě se používá pseudoinverze
nebo regularizace a u velkých dat je vzorec drahý.\par
...
\begin{odpoved}
\setlength{\parskip}{2pt}
\noindent\bod{1}\ \textbf{Minimum:} \emph{Rozhodovací strom} má ve vnitřním uzlu test příznaku, hrany podle výsledku testu a v listu predikci.
\emph{Entropie} uzlu je $H(S)=-\sum_c p_c\log p_c$ a \emph{Gini nečistota} je $G(S)=1-\sum_c p_c^2$.\par
\noindent\bod{2}\ \textbf{+ k tomu:} V každém uzlu vyber dělení, které nejvíc \emph{zvýší čistotu} -
\emph{informační zisk} je pokles entropie po rozdělení. Rekurzivně až do
...
\begin{odpoved}
\setlength{\parskip}{2pt}
\noindent\bod{1}\ \textbf{Minimum:} \emph{SVM} hledá nadrovinu $w^\top x+b=0$ oddělující třídy s maximálním marginem; pro kanonické škálování je šířka marginu $\frac{2}{\|w\|}$.
Klasifikace je $\operatorname{sign}(w^\top x+b)$.\par
...
nezávislá pozorování a zhruba normální data (Welchova varianta nepotřebuje stejné rozptyly).
\emph{Chí-kvadrát test dobré shody} porovnává \emph{pozorované vs.\ očekávané
...
t-test testuje rozdíly dvojic jako jednovýběrový. Chí-kvadrát dobré shody má
počet kategorií minus jedna, případně minus počet odhadnutých parametrů, stupňů volnosti.
\end{odpoved}
\begin{recall}
\begin{itemize}[leftmargin=1.2em,itemsep=0pt,topsep=1pt]
...
\begin{odpoved}
\setlength{\parskip}{2pt}
\noindent\bod{1}\ \textbf{Minimum:} \emph{k-means} minimalizuje $\sum_{j=1}^k\sum_{x_i\in C_j}\|x_i-\mu_j\|^2$, kde $\mu_j$ je centroid shluku.
\emph{PCA} hledá ortogonální směry maximalizující rozptyl projekce; jsou to vlastní vektory kovarianční matice s největšími vlastními čísly.\par
