...
$\eta$ = rychlost učení (moc velká diverguje, moc malá je pomalá).
\end{odpoved}
\begin{recall}
\begin{itemize}[leftmargin=1.2em,itemsep=0pt,topsep=1pt]
...
chybu na validaci). Výhoda: interpretovatelnost.
\end{odpoved}
\begin{recall}
\begin{itemize}[leftmargin=1.2em,itemsep=0pt,topsep=1pt]
...
více tříd přes one-vs-rest nebo one-vs-one.
\end{odpoved}
\begin{recall}
\begin{itemize}[leftmargin=1.2em,itemsep=0pt,topsep=1pt]
...
příkladů; silný, ale citlivější na šum/přeučení než bagging.
\end{odpoved}
\begin{recall}
\begin{itemize}[leftmargin=1.2em,itemsep=0pt,topsep=1pt]
...
chyba II.\ druhu = nezamítnu neplatné $H_0$.
\end{odpoved}
\begin{recall}
\begin{itemize}[leftmargin=1.2em,itemsep=0pt,topsep=1pt]
...
\begin{tikzpicture}[scale=0.8]
  ...
  \node[font=\scriptsize,gapred] at (1.7,0.2) {$\times$ = centroidy};
  \draw[->,thick,defblue] (1.4,0.7) -- (2.6,2.0) node[midway,sloped,above,font=\scriptsize]{hlavní komponenta};
\end{tikzpicture}
\end{center}
