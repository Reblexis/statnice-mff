...
\end{tema}
\begin{vezkratce}
\zk{Definice}{Učení s učitelem hledá z označených dat funkci, která dobře generalizuje.}
\zk{Spouštěč}{train, validation, test, supervised, klasifikace, regrese, regularizace, metrika}
\zk{Šablona}{Rozděl data - natrénuj model - laď validací - reportuj test a regularizaci.}
\zk{Past}{Testovací data se nesmí používat pro výběr modelu ani ladění hyperparametrů.}
\end{vezkratce}
...
F_1=\frac{2PR}{P+R}.
\]
U regrese je základní metrika $\mathrm{MSE}=\frac1N\sum_{i=1}^N(y_i-t_i)^2$, kde $y_i$ je predikce a $t_i$ skutečná hodnota. Často se používají také
\[
\mathrm{RMSE}=\sqrt{\frac1N\sum_i(y_i-t_i)^2},\qquad
\mathrm{MAE}=\frac1N\sum_i|y_i-t_i|.
\]
RMSE trestá velké chyby silněji, MAE je robustnější vůči odlehlým hodnotám.\par
...
\]
ROC křivka vzniká posouváním prahu pro pozitivní výsledek: na ose $x$ je $\mathrm{FPR}=\frac{FP}{FP+TN}$, na ose $y$ je $\mathrm{TPR}=\frac{TP}{TP+FN}=R$. Náhodný klasifikátor má diagonálu a AUC $0{,}5$, dokonalý klasifikátor má bod vlevo nahoře $(0,1)$ a AUC $1$. Senzitivita je TPR, specificita je $\mathrm{TNR}=\frac{TN}{TN+FP}$; obecně mají metriky typu rate ve jmenovateli všechny instance se stejnou skutečnou třídou jako čitatel. Pokud je v testu $P_0$ skutečně pozitivních a $N_0$ skutečně negativních instancí, pak
\[
TP=\mathrm{TPR}\cdot P_0,\quad FN=(1-\mathrm{TPR})P_0,\quad
FP=\mathrm{FPR}\cdot N_0,\quad TN=(1-\mathrm{FPR})N_0.
\]
Z pevné precision $q$ plyne $q=TP/(TP+FP)$, tedy $FP=TP(1-q)/q$; to dovoluje dopočítat bod ROC i matici konfuze z nevyvážených dat.\par
...
Při maximální věrohodnosti fixujeme trénovací dvojice a hledáme váhy
\[
L(w)=p_{\mathrm{model}}(t\mid X;w)=\prod_i p_{\mathrm{model}}(t_i\mid x_i;w),\qquad
w_{\mathrm{MLE}}=\arg\max_w L(w).
\]
...
U tabulárních dat se nejprve rozliší číselné a kategorické příznaky. Kategorické příznaky se pro lineární modely, kNN, SVM a neuronové modely typicky převádějí one-hot kódováním. Číselné příznaky se škálují standardizací nebo normalizací, pokud je model citlivý na měřítko, například kNN, lineární model, logistická regrese, SVM nebo PCA; stromové modely a náhodné lesy škálování obvykle nevyžadují. Chybějící číselné hodnoty lze doplnit průměrem nebo mediánem, kategorické modem nebo zvláštní kategorií. Pro tabulární regresi bývají silným konzervativním modelem náhodné lesy nebo gradient boosting; lineární regrese je vhodná jako jednoduchý a interpretovatelný baseline.
\end{odpoved}
...
\end{tema}
\begin{vezkratce}
\zk{Definice}{kNN predikuje podle nejbližších trénovacích příkladů v zadané metrice.}
\zk{Spouštěč}{nejbližší soused, $k$, metrika, lazy learning, hlasování, vzdálenost}
\zk{Šablona}{Zvol metriku - najdi $k$ sousedů - hlasuj nebo průměruj - validuj $k$.}
\zk{Past}{Bez škálování příznaků může vzdálenost ovládnout jediná veličina.}
\end{vezkratce}
...
\]
typicky pro konečné $p\in\set{1,2,3}$; pro $p=\infty$ je $\|x\|_\infty=\max_i |x_i|$. Malé $k$ je citlivé na šum a vede k přeučení, velké $k$ hranice vyhlazuje a může podtrénovat; $k$ se volí validací. Příznaky je nutné škálovat, jinak dominuje rozměr s velkým rozsahem. Nevýhody: drahá predikce přes mnoho uložených bodů a silné prokletí dimenzionality.
\end{odpoved}
\begin{recall}
\begin{itemize}[leftmargin=1.2em,itemsep=0pt,topsep=1pt]
...
\end{tema}
\begin{vezkratce}
\zk{Definice}{Lineární regrese aproximuje spojitou cílovou hodnotu lineární kombinací příznaků.}
\zk{Spouštěč}{OLS, MSE, koeficient, gradient, ridge, lasso, reziduum}
\zk{Šablona}{Zapiš $Xw$ - minimalizuj MSE - vyřeš normálně nebo gradientem - regularizuj.}
\zk{Past}{Korelované příznaky zhoršují interpretaci koeficientů i stabilitu odhadu.}
\end{vezkratce}
...
\begin{itemize}[leftmargin=1.2em,itemsep=0pt,topsep=1pt]
  ...
  \item Co řídí parametr $\alpha$ u SGD?
\end{itemize}
\end{recall}
...
\end{tema}
\begin{vezkratce}
\zk{Definice}{Logistická regrese modeluje pravděpodobnost třídy sigmoidem lineárního skóre.}
\zk{Spouštěč}{sigmoid, logit, log-loss, odds, binární klasifikace, rozhodovací hranice}
\zk{Šablona}{Spočti lineární skóre - použij sigmoid - optimalizuj log-loss - prahuj.}
\zk{Past}{Výstup je pravděpodobnost třídy, ne přímo lineární regresní hodnota.}
\end{vezkratce}
...
\end{tema}
\begin{vezkratce}
\zk{Definice}{Rozhodovací strom rekurzivně dělí prostor příznaků podle testů v uzlech.}
\zk{Spouštěč}{strom, split, entropy, Gini, information gain, pruning, list}
\zk{Šablona}{Vyber kritérium - rozděl uzel - zastav nebo prořež - predikuj listem.}
\zk{Past}{Hluboký neprořezaný strom se snadno přeučí na trénovací data.}
\end{vezkratce}
...
\end{tema}
\begin{vezkratce}
\zk{Definice}{SVM hledá separující nadrovinu s maximální rezervou, případně v kernelovém prostoru.}
\zk{Spouštěč}{margin, support vector, soft-margin, $C$, kernel, separace, nadrovina}
\zk{Šablona}{Zapiš margin - dovol slack přes $C$ - použij kernel - klasifikuj znaménkem.}
\zk{Past}{Kernel implicitně mění prostor příznaků, neznamená jen jinou metriku.}
\end{vezkratce}
...
Více tříd se řeší kombinací binárních klasifikátorů. One-versus-rest natrénuje $K$ klasifikátorů "třída proti zbytku" a vybírá největší skóre nebo pravděpodobnost; u SVM se také používá, ale skóre může vyžadovat kalibraci. One-versus-one trénuje klasifikátor pro každou dvojici tříd; klasifikátorů je více, ale každý vidí méně dat, a predikce probíhá většinovým hlasováním.
\end{odpoved}
\begin{recall}
\begin{itemize}[leftmargin=1.2em,itemsep=0pt,topsep=1pt]
...
\end{tema}
\begin{vezkratce}
\zk{Definice}{Ensembly kombinují více modelů, aby snížily varianci, bias nebo využily různé chyby.}
\zk{Spouštěč}{bagging, random forest, boosting, stacking, AdaBoost, variance, bias}
\zk{Šablona}{Urči způsob kombinace - natrénuj členy - agreguj hlasováním, průměrem nebo metaučením.}
\zk{Past}{Boosting není totéž co bagging, modely se učí postupně na těžších příkladech.}
\end{vezkratce}
...
\end{tema}
\begin{vezkratce}
\zk{Definice}{Statistický test rozhoduje, zda data poskytují dost důkazů proti nulové hypotéze.}
\zk{Spouštěč}{hypotéza, $p$-hodnota, hladina, t-test, chyba I, chyba II, síla}
\zk{Šablona}{Stanov $H_0,H_1$ - zvol statistiku - spočti $p$ - porovnej s $\alpha$.}
\zk{Past}{$p$-hodnota není pravděpodobnost, že nulová hypotéza je pravdivá.}
\end{vezkratce}
...
\noindent\bod{3}\ \textbf{Bonus:} Dvouvýběrový t-test porovnává střední hodnoty dvou nezávisle samplovaných populací, například $H_0:\mu_X=\mu_Y$, i s různými počty vzorků a při přibližné normalitě nebo dost velkých výběrech. Welchova varianta pro neznámé a obecně různé rozptyly používá
\[
T=\frac{\overline{X_n}-\overline{Y_m}}{\mathrm{se}},
\qquad
\mathrm{se}=\sqrt{\frac{\bar\sigma_X^2}{n}+\frac{\bar\sigma_Y^2}{m}},
\]
případně interval přes $\delta=\mathrm{se}\cdot\Psi_\nu^{-1}(1-\alpha/2)$ s Welchovými stupni volnosti $\nu$. Při předpokladu stejných rozptylů se používá pooled variance a t-rozdělení s $n+m-2$ stupni volnosti. Párový t-test se používá pro související dvojice dat: vezmeme rozdíly $R_i=Y_i-X_i$ a použijeme jednovýběrový test na rozdíly.
...
\]
Typická úloha je spravedlivost kostky; pro šest kategorií použijeme $\chi^2$ rozdělení s pěti stupni volnosti. Obecně $H_0$ říká, že pozorované četnosti odpovídají očekávaným, a pokud $H_0$ platí a neodhaduje se žádný parametr z dat, má statistika $\chi^2$ rozdělení s $n-1$ stupni volnosti pro $n$ kategorií. $H_0$ zamítáme, když je statistika větší než kritická hodnota.
\end{odpoved}
\begin{recall}
\begin{itemize}[leftmargin=1.2em,itemsep=0pt,topsep=1pt]
...
\end{tema}
\begin{vezkratce}
\zk{Definice}{Učení bez učitele hledá strukturu v neoznačených datech, typicky shluky nebo projekce.}
\zk{Spouštěč}{clustering, k-means, hierarchické shlukování, PCA, komponenta, variance}
\zk{Šablona}{Zvol reprezentaci - škáluj data - najdi shluky nebo komponenty - interpretuj.}
\zk{Past}{Shluky a komponenty nemusí odpovídat skutečným třídám ani kauzálním skupinám.}
\end{vezkratce}
...
PCA dimenze $M$ z datové matice $X$: spočti průměr řádků $\bar x$, proveď SVD matice $X-\bar x$, nech prvních $M$ řádků $V^T$ (ekvivalentně $M$ sloupců $V$) a nové komponenty získej jako $(X-\bar x)V_M$, matici $N\times M$. Funguje to proto, že SVD centrovaných dat dává vlastní vektory kovarianční matice
\[
