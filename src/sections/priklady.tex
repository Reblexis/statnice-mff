...
int ReadU16BE(byte[] b, int i) => (b[i] << 8) | b[i + 1];
// 16bit signed: dvojkový doplněk
short ReadS16BE(byte[] b, int i) => (short)((b[i] << 8) | b[i + 1]);

class Reader {
    private byte[] data; private int attrCount;
    public Reader(BinaryFile f) {        // konstruktor načte hlavičku
        byte[] h = new byte[2]; f.readBytes(0, h, 2);
        attrCount = ReadU16BE(h, 0);
    }
...
\begin{verbatim}
SemaphoreSlim empty = new(capacity);   // volná místa v bufferu
SemaphoreSlim full  = new(0);          // obsazená místa
object mtx = new();
Queue<int> buf = new();
