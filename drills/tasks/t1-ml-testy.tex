...
\textbf{(c)} \emph{Párový} t-test se použije, jsou-li data \emph{spárovaná} (např.\ měření před/po na týchž jedincích); testuje, zda je průměr rozdílu nulový. \emph{Chyba 1.\ druhu} = zamítnout platné $H_0$ (řízena $\alpha$); \emph{chyba 2.\ druhu} = nezamítnout neplatné $H_0$, má pravděpodobnost $\beta$; \emph{síla testu} $1-\beta$ roste s velikostí vzorku a velikostí efektu (tedy $\beta$ klesá).
\end{reseni}
