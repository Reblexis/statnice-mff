...
\begin{reseni}
\textbf{(a)} $F$ je \emph{primitivní funkce} k $f$ na intervalu, je-li $F'=f$. \emph{Riemannuv integrál} $\int_a^b f$ je společná limita horních a dolních součtu přes zjemňující se dělení (existuje, je-li $f$ např.\ spojitá). \emph{Newton-Leibniz:} je-li $F$ primitivní k spojité $f$, pak $\int_a^b f = F(b)-F(a)$. \emph{Geometrická řada:} pro $a\neq0$ je $\sum_{n=0}^\infty ar^n=\frac{a}{1-r}$ při $|r|<1$ a jinak diverguje (pro $a=0$ je součet $0$); částečný součet $\sum_{n=0}^{N}ar^n=a\frac{1-r^{N+1}}{1-r}$.
