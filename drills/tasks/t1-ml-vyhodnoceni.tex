...
\begin{enumerate}[label=(\alph*)]
  ...
  \item \bod{3} Vysvětlete vztah podučení vs přeučení (jednoduchý vs složitý model) a prokletí dimenzionality při výběru modelu.
\end{enumerate}
\end{zadani}
...
\textbf{(c)} \emph{Podučení vs přeučení}: příliš jednoduchý model špatně zachytí vztah (podučení - velká chyba i na tréninku), příliš složitý se učí šum (přeučení - malá trénovací, velká testovací chyba); cílem je rovnováha mezi nimi. \emph{Prokletí dimenzionality}: s rostoucím počtem příznaku roste potřeba dat exponenciálně a vzdálenosti se vyrovnávají, takže složitější modely snáz přeučí - proto redukce dimenze/regularizace.
\end{reseni}

\begin{jadro}
Train (učení) / dev (ladění) / test (jediný závěrečný odhad); $k$-fold CV průměruje. Přeučení = malá train, velká test chyba. L2 hladce smršťuje, L1 dělá řídké váhy (výběr příznaku). Hledej rovnováhu podučení/přeučení.
\end{jadro}
