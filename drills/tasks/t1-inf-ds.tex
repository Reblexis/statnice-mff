...
\textbf{(c)} \emph{Successor} (nejbližší větší): má-li uzel pravý podstrom, je to jeho nejlevější (nejmenší) uzel; jinak jdi nahoru, dokud nejsi levým synem rodiče (ten je následník). U \emph{nevyváženého} BVS je $h$ až $O(n)$ (degenerace na seznam), takže operace $O(n)$; u \emph{vyváženého} (AVL) $O(\log n)$. Haldu lze postavit \emph{zdola nahoru} (\textsc{Heapify} od posledního vnitřního uzlu): součet prací $\sum_h \tfrac{n}{2^{h+1}}\cdot O(h)=O(n)$.
\end{reseni}
