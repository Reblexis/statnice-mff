...
\begin{enumerate}[label=(\alph*)]
  ...
  \item \bod{3} Uveďte, kdy je $\mathbb{Z}_n$ těleso. \emph{(Volitelně: Lagrangeova věta.)}
\end{enumerate}
\end{zadani}
...
\textbf{(c)} $\mathbb{Z}_n$ (zbytky modulo $n$) je \emph{těleso právě tehdy, když je $n$ prvočíslo} - jen tehdy má každý nenulový prvek multiplikativní inverz (jinak existují dělitelé nuly). \emph{(Nad rámec požadavku - Lagrangeova věta: v konečné grupě řád podgrupy dělí řád grupy, $|H|\mid|G|$; důsledek: řád prvku dělí $|G|$.)}
\end{reseni}
