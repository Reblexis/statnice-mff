...
\]
(off-policy). \emph{Explorace vs exploitace}: je třeba občas zkusit i nepreferované akce (např.\ $\varepsilon$-greedy), jinak agent uvázne v suboptimu; SARSA je on-policy obdoba (místo $\max$ použije skutečně zvolenou $a'$). \emph{POMDP} (částečně pozorovatelný MDP): agent stav přímo nevidí, udržuje \emph{belief} (rozdělení pravděpodobnosti nad stavy), které po akci a pozorování aktualizuje (Bayes), a rozhoduje se podle něj.
\end{reseni}

\begin{jadro}
MDP $=(S,A,P,R,\gamma)$. Bellman: $V^*(s)=\max_a\sum_{s'}P(s'\!\mid\!s,a)[R+\gamma V^*(s')]$; iterace hodnot aplikuje toto jako update. Q-učení: $Q(s,a)\leftarrow Q+\alpha[r+\gamma\max_{a'}Q(s',a')-Q]$.
\end{jadro}
